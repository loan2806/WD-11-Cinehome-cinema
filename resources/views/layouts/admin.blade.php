...
<body class="overflow-x-hidden bg-[#080808] text-white">
    ...
    <div id="adminLayout" data-sidebar="open" class="min-h-screen overflow-x-hidden bg-[#080808] text-white">
        ...
        {{-- SIDEBAR PREMIUM - CINEHOME CORE ARCHITECTURE --}}
        <aside id="adminSidebar" class="admin-scrollbar fixed left-0 top-0 z-[60] h-screen w-[285px] overflow-y-auto overflow-x-hidden border-r border-[#d99a32]/20 bg-gradient-to-b from-[#110702] to-[#200d05] transition-transform duration-300">
            ...
            <div class="flex items-center gap-3.5 px-5 py-6 border-b border-white/5">
                ...
                <div>
                    ...
                    <p class="text-xs font-black tracking-widest text-[#f4c56a] uppercase mt-0.5">
                        Root Controller
                    </p>
                </div>
            </div>

            {{-- MENU THANH ĐIỀU HƯỚNG DROPDOWN COMPACT --}}
            <div class="mt-5 px-3 pb-8 space-y-4">

                {{-- MỤC TĨNH: DASHBOARD TỔNG QUAN --}}
                <div>
                    <p class="mb-2 px-3 text-[11px] font-black uppercase tracking-widest text-[#d7a767]/50">Tổng quan hệ
                        thống</p>
                    <a href="{{ route('admin.dashboard') }}" class="{{ request()->routeIs('admin.dashboard') ? 'bg-[#d99a32] text-[#2b1208]' : 'text-gray-300 hover:bg-white/5' }} no-underline flex items-center gap-3.5 px-4 py-3 rounded-xl text-[16px] font-bold transition duration-200">
                ...
                <div class="sidebar-dropdown-box">
                    <button type="button" class="sidebar-dropdown-btn w-full flex items-center justify-between px-4 py-3 rounded-xl text-[16px] font-black text-[#f4c56a] bg-white/5 hover:bg-white/10 transition duration-200 border border-[#d99a32]/30 text-left whitespace-nowrap leading-none outline-none">
                    ...
                    <div class="sidebar-dropdown-content pl-5 mt-1 border-l-2 border-[#d99a32] ml-6 space-y-1">
                        ...
                        <a href="#" class="block py-2.5 pl-3 text-[15px] font-bold text-gray-400 hover:text-white transition duration-200 hover:translate-x-1.5 no-underline">Cài
                            đặt cổng thanh toán</a>
                        <a href="#" class="block py-2.5 pl-3 text-[15px] font-bold text-gray-400 hover:text-white transition duration-200 hover:translate-x-1.5 no-underline">Sao
                            lưu dữ liệu hạt nhân</a>
                        <a href="#" class="block py-2.5 pl-3 text-[15px] font-bold text-gray-400 hover:text-white transition duration-200 hover:translate-x-1.5 no-underline">Log
                ...
                @php
                $isNoidungActive = request()->routeIs('admin.phims.*') || request()->routeIs('admin.suat-chieus.*') ||
                request()->routeIs('admin.the-loais.*') || request()->routeIs('admin.quoc-gias.*');
                @endphp
                ...
                <div class="sidebar-dropdown-box {{ $isGiaoDichActive ? 'open' : '' }}">
                    ...
                    <div class="sidebar-dropdown-content pl-5 mt-1 border-l-2 border-[#d99a32]/20 ml-6 space-y-1">
                        @can('ban_ve_tai_quay')
                        <a href="{{ route('admin.ve-xem-phims.index') }}" class="block py-2.5 pl-3 text-[15px] font-semibold transition duration-200 hover:translate-x-1.5 {{ request()->routeIs('admin.ve-xem-phims.*') ? 'text-[#d99a32]' : 'text-gray-400 hover:text-white' }} no-underline">Quản
                            lý kho dữ liệu vé</a>
                        <a href="#" class="block py-2.5 pl-3 text-[15px] font-semibold text-gray-400 hover:text-white transition duration-200 hover:translate-x-1.5 no-underline">Bán
                            vé trực tiếp rạp</a>
                        @endcan
                        @can('quan_ly_do_an_combo')
                        <a href="{{ route('admin.food-invoices.index') }}" class="block py-2.5 pl-3 text-[15px] font-semibold transition duration-200 hover:translate-x-1.5 {{ request()->routeIs('admin.food-invoices.*') ? 'text-[#d99a32]' : 'text-gray-400 hover:text-white' }} no-underline">Hóa
                            đơn đồ ăn & Combo</a>
                        @endcan
                        <a href="#" class="block py-2.5 pl-3 text-[14px] font-medium text-gray-500 hover:text-[#d99a32] transition duration-200 hover:translate-x-1.5 no-underline">+
                ...
                @if(auth()->user()->can('quan_ly_khach_hang') || auth()->user()->can('quan_ly_nhan_vien') ||
                auth()->user()->can('phan_quyen_he_thong'))
                @php
                $isTaiKhoanActive = request()->routeIs('admin.nhanviens.*') || request()->routeIs('admin.phan-quyen.*');
                @endphp
                <div class="sidebar-dropdown-box {{ $isTaiKhoanActive ? 'open' : '' }}">
                    ...
                    <div class="sidebar-dropdown-content pl-5 mt-1 border-l-2 border-[#d99a32]/20 ml-6 space-y-1">
                        ...
                        <a href="{{ route('admin.nhanviens.index') }}" class="block py-2.5 pl-3 text-[15px] font-semibold transition duration-200 hover:translate-x-1.5 {{ request()->routeIs('admin.nhanviens.*') ? 'text-[#d99a32]' : 'text-gray-400 hover:text-white' }} no-underline">Danh
                            sách nhân viên</a>
                        @endcan
                        @can('phan_quyen_he_thong')
                        <a href="{{ route('admin.phan-quyen.index') }}" class="block py-2.5 pl-3 text-[15px] font-semibold transition duration-200 hover:translate-x-1.5 {{ request()->routeIs('admin.phan-quyen.*') ? 'text-[#d99a32]' : 'text-gray-400 hover:text-white' }} no-underline">Ma
                            trận phân quyền</a>
                        @endcan
                        @can('quan_ly_khach_hang')
                        <a href="{{ route('admin.khach-hang.index') }}" class="block py-2.5 pl-3 text-[15px] font-semibold transition duration-200 hover:translate-x-1.5 no-underline
                            {{ request()->routeIs('admin.khach-hang.*') ? 'text-[#d99a32]' : 'text-gray-400 hover:text-white' }}">
                            Tài khoản khách hàng
                        </a>
                        @endcan
                        <a href="{{ route('admin.thanh-vien.index') }}" class="block py-2.5 pl-3 text-[14px] font-medium transition duration-200 hover:translate-x-1.5 no-underline
                            {{ request()->routeIs('admin.thanh-vien.*') ? 'text-[#d99a32]' : 'text-gray-500 hover:text-[#d99a32]' }}">
                ...
                request()->routeIs('admin.movie-reviews.*') || request()->routeIs('admin.system-settings.*');
                @endphp
                <div class="sidebar-dropdown-box {{ $isSystemActive ? 'open' : '' }}">
                    ...
                    <div class="sidebar-dropdown-content pl-5 mt-1 border-l-2 border-[#d99a32]/20 ml-6 space-y-1">
                        <a href="{{ route('admin.notifications.index') }}" class="block py-2.5 pl-3 text-[15px] font-semibold transition duration-200 hover:translate-x-1.5 {{ request()->routeIs('admin.notifications.*') ? 'text-[#d99a32]' : 'text-gray-400 hover:text-white' }} no-underline">Thông
                            báo đẩy</a>
                        <a href="{{ route('admin.movie-reviews.index') }}" class="block py-2.5 pl-3 text-[15px] font-semibold transition duration-200 hover:translate-x-1.5 {{ request()->routeIs('admin.movie-reviews.*') ? 'text-[#d99a32]' : 'text-gray-400 hover:text-white' }} no-underline">Kiểm
                            duyệt đánh giá</a>
                ...
                {{-- LỐI THOÁT RA TRANG GIAO DIỆN CHỦ --}}
                <div class="pt-3 border-t border-white/10">
                    <a href="{{ route('home') }}" class="flex items-center gap-3.5 rounded-xl px-4 py-3 text-[16px] font-bold text-gray-300 hover:bg-white/5 hover:text-white transition duration-200 no-underline">
        ...
        {{-- MAIN CONTENT AREA --}}
        <main id="adminMain" class="min-h-screen overflow-x-hidden bg-[#080808] ml-[285px]">
            ...
            <header class="sticky top-0 z-50 border-b border-white/10 bg-[#101010]/95 backdrop-blur-xl">
                <div class="flex h-[76px] items-center justify-between gap-4 px-5">
                    ...
                    <div class="flex min-w-0 items-center gap-4">
                        <button id="sidebarToggle" type="button" class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white/10 text-white transition hover:bg-[#d99a32] hover:text-[#2b1208] border-0">
                    ...
                    <div class="hidden md:flex h-11 w-full max-w-[280px] lg:max-w-[360px] items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-4">
                        ...
                        <input type="text" placeholder="Tìm nhanh chức năng hệ thống..." class="h-full w-full bg-transparent text-sm text-white outline-none placeholder-gray-500 border-0">
                    </div>

                    {{-- RIGHT: BOX THÔNG TIN USER --}}
                    <div class="flex items-center gap-3">
                        @auth
                        <div class="relative" id="adminDropdownBox">
                            <button type="button" id="adminDropdownBtn" class="inline-flex items-center gap-3 rounded-2xl bg-white/10 px-4 py-2 transition hover:bg-white/15 border-0">
                                ...
                                <div class="hidden sm:block text-left">
                                    ...
                                    <div class="text-xs text-[#d99a32] font-semibold mt-0.5">
                                        {{ Auth::user()->roles->pluck('name')->first() ?? 'Quản lý hệ thống' }}
                                    </div>
                                </div>
    ...
    {{-- INTERACTION SCRIPT: ACCORDION ĐỘC LẬP MULTI-OPEN --}}
    <script>
