<x-guest-layout>
    ...
    {{-- ĐỊNH VỊ KHUNG CHÍNH GIỮA MÀN HÌNH --}}
        ...
        <div class="w-full max-w-[500px] rounded-2xl border border-white/10 bg-[#151515]/95 p-10 shadow-2xl backdrop-blur-md">
            ...
            <div class="text-center mb-8">
                ...
                <p class="mt-2 text-sm text-gray-400 uppercase tracking-widest font-semibold">
                    Hệ thống điều phối & quản lý rạp phim
                </p>
            </div>

            {{-- THANH ĐIỀU HƯỚNG DẠNG TAB --}}
            <div class="flex rounded-xl bg-[#2a2a2a]/60 p-1.5 mb-6 border border-white/5">
                <a href="{{ route('login') }}" class="w-1/2 text-center py-2.5 text-sm font-black rounded-lg bg-gradient-to-r from-[#8a4a21] to-[#d99a32] text-white text-decoration-none shadow-md">
                    Đăng nhập
                </a>
                <a href="{{ route('register') }}" class="w-1/2 text-center py-2.5 text-sm font-bold text-gray-400 hover:text-white transition-colors text-decoration-none flex items-center justify-center">
                    Đăng ký
                </a>
            </div>
            ...
            <form method="POST" action="{{ route('login') }}" id="loginForm" class="space-y-6 m-0">
                ...
                <div class="space-y-2">
                    ...
                    <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus autocomplete="username" placeholder="admin@cinehome.vn" class="block h-12 w-full rounded-xl border border-[#8a4a21] bg-[#2a2a2a]/60 px-4 text-base text-white outline-none focus:border-[#d99a32] focus:ring-1 focus:ring-[#d99a32] transition-colors shadow-inner" />
                    @if($errors->has('email'))
                ...
                <div class="flex items-center justify-between pt-1">
                    ...
                    @if (Route::has('password.request'))
                        <a class="text-sm text-gray-400 hover:text-[#d99a32] transition-colors text-decoration-none font-medium" href="{{ route('password.request') }}">
                            Quên mật khẩu?
                        </a>
                    @endif
                </div>
