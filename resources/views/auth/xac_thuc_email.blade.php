<x-guest-layout>
    ...
    <div class="fixed inset-0 z-10 flex h-screen w-screen overflow-hidden items-center justify-center px-4">
        <div class="w-full max-w-[500px] rounded-2xl border border-white/10 bg-[#151515]/95 p-10 shadow-2xl backdrop-blur-md">
            ...
            {{-- THÔNG ĐIỆP CHÀO MỪNG THÀNH VIÊN MỚI --}}
            <div class="text-center mb-6">
                ...
                <p class="mt-3 text-sm text-gray-400 leading-relaxed font-normal tracking-wide text-center">
                    Cảm ơn bạn đã đăng ký thành viên tại <b class="text-white">CineHome</b>! Trước khi bắt đầu trải nghiệm đặt vé xem phim, vui lòng kích hoạt tài khoản bằng cách nhấn vào liên kết xác thực chúng tôi vừa gửi vào hộp thư Email của bạn.
                </p>
            </div>

            {{-- ĐOẠN PHẢN HỒI THÀNH CÔNG KHI BẤM GỬI LẠI LINK --}}
            @if (session('status') == 'verification-link-sent')
                <div class="mb-5 p-4 rounded-xl text-sm font-semibold text-green-400 bg-green-500/10 border border-green-500/20 leading-relaxed">
                    <i class="fa-solid fa-circle-check mr-1"></i> Một liên kết xác thực mới đã được hệ thống tạo lập và gửi thành công tới địa chỉ Email đăng ký của bạn.
                </div>
            @endif
            ...
            <div class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-white/10 pt-5">
                ...
                <form method="POST" action="{{ route('verification.send') }}" class="w-full sm:w-auto m-0">
                    @csrf
                    <button type="submit" class="w-full sm:w-auto h-11 px-5 rounded-xl font-bold text-sm transition-all bg-gradient-to-r from-[#8a4a21] to-[#d99a32] text-white shadow-lg border-0 cursor-pointer hover:from-[#d99a32] hover:to-[#8a4a21] active:scale-[0.98] flex items-center justify-center gap-2">
                ...
                <form method="POST" action="{{ route('logout') }}" class="w-full sm:w-auto m-0">
                    @csrf
                    <button type="submit" class="w-full sm:w-auto h-11 px-4 rounded-xl text-sm font-bold text-gray-400 hover:text-red-400 border border-white/10 bg-white/5 transition hover:bg-red-500/15 hover:border-red-500/20 cursor-pointer flex items-center justify-center gap-1.5">
