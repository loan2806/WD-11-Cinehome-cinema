            ...
            <div class="grid lg:grid-cols-[380px_1fr]">
                ...
                <div class="border-r border-white/10 p-8">
                    ...
                    <div class="border-t border-white/10 pt-6 mt-6">
                        ...
                        <div class="mt-4 text-sm space-y-2 text-gray-300">
                            ...
                            <div><strong>Suất chiếu:</strong> {{ $suatChieu->thoi_gian_chieu->format('d/m/Y H:i') }}
                            </div>
                        </div>
                ...
                <div class="p-8">
                    ...
                    <div class="pt-6">
                        ...
                        <form action="{{ route('dat_ve.xu_ly_thanh_toan', $suatChieu->id) }}" method="POST">
                            @csrf
                            ...
                            <div class="space-y-3">
                                ...
                                <label
                                    class="payment-method-label flex items-center justify-between border border-yellow-400 bg-yellow-400/10 p-4 rounded-2xl cursor-pointer transition dynamics-duration-200">
                                    <div class="flex items-center gap-3">
                                        <input type="radio" checked name="payment_method" value="online"
                                            class="accent-yellow-400 w-5 h-5">
                                        <div class="ml-1">
                                            <span class="block font-bold text-gray-100">Cổng thanh toán VNPAY</span>
                                            <span class="block text-xs text-gray-400 mt-0.5">Thanh toán qua ATM, Thẻ
                                                quốc tế hoặc ứng dụng ngân hàng</span>
                                        </div>
                                    </div>
                                    <div
                                        class="bg-white px-2 py-1 rounded-lg flex items-center justify-center shadow-sm">
                                        <img src="{{ asset('assets/images/logo-vnpay.webp') }}"
                                            class="h-5 w-16 object-contain" alt="VNPAY Logo">
                                    </div>
                                </label>

                                {{-- LỰA CHỌN 2: CHUYỂN KHOẢN VIETQR --}}
                                <label
                                    class="payment-method-label flex items-center justify-between border border-white/10 bg-zinc-900/30 p-4 rounded-2xl cursor-pointer transition dynamics-duration-200 hover:border-white/20">
                                    <div class="flex items-center gap-3">
                                        <input type="radio" name="payment_method" value="vietqr"
                                            class="accent-yellow-400 w-5 h-5">
                                        <div class="ml-1">
                                            <span class="block font-bold text-gray-200">Chuyển khoản nhanh VietQR</span>
                                            <span class="block text-xs text-gray-400 mt-0.5">Tạo mã QR bốc động số tiền,
                                                quét để chuyển khoản an toàn</span>
                                        </div>
                                    </div>
                                    <div
                                        class="bg-white px-2 py-1 rounded-lg flex items-center justify-center shadow-sm">
                                        <img src="{{ asset('assets/images/logo-vietqr.png') }}"
                                            class="h-5 w-16 object-contain" alt="VietQR Logo">
                                    </div>
                                </label>
                            </div>
