...
<div class="mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
    <div class="stat-card">
        ...
        <div class="stat-value">{{ number_format((float) $summary['ticket_revenue'], 0, ',', '.') }}đ</div>
    </div>
    <div class="stat-card">
        <div class="stat-label">Doanh thu đồ ăn</div>
        <div class="stat-value">{{ number_format((float) $summary['food_revenue'], 0, ',', '.') }}đ</div>
    </div>
    <div class="stat-card">
        <div class="stat-label">Tổng doanh thu</div>
        <div class="stat-value">{{ number_format((float) $summary['total_revenue'], 0, ',', '.') }}đ</div>
    </div>
    <div class="stat-card">
        <div class="stat-label">Vé / hóa đơn</div>
        <div class="stat-value">{{ $summary['tickets_sold'] }} / {{ $summary['food_invoices'] }}</div>
    </div>
</div>
            ...
            <table class="admin-table">
                ...
                <tbody>
                    @forelse ($tickets as $ticket)
                        <tr>
                            <td>{{ $ticket->ma_ve }}</td>
                            <td>{{ $ticket->ten_phim ?? 'Không rõ phim' }}</td>
                            <td>{{ $ticketStatusLabels[$ticket->trang_thai] ?? $ticket->trang_thai }}</td>
                            <td>{{ number_format((float) $ticket->tong_tien, 0, ',', '.') }}đ</td>
                        </tr>
                    @empty
                        <tr>
                ...
                <tbody>
                    @forelse ($foodInvoices as $invoice)
                        <tr>
                            ...
                            <td>{{ $foodStatusLabels[$invoice->payment_status] ?? $invoice->payment_status }}</td>
                            <td>{{ number_format((float) $invoice->total, 0, ',', '.') }}đ</td>
                        </tr>
                    @empty
                        <tr>
