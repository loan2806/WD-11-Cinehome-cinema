...
<div class="member-admin-page member-detail-page">
    ...
    <section class="member-point-forms">
        <article class="member-point-form is-add">
            ...
            <form method="POST" action="{{ route('admin.thanh-vien.tang-diem', $thanhVien) }}">
                @csrf
                <label>
                    <span>Số điểm</span>
                    <input type="number" name="so_diem" min="1" max="10000" placeholder="VD: 100" required>
                </label>

                <label>
                    <span>Lý do tặng điểm</span>
                    <textarea name="noi_dung" rows="3" placeholder="VD: Đền bù lỗi thanh toán" required></textarea>
                </label>

                <label>
                    <span>Cách tính hạng</span>
                    <select name="tinh_vao_hang" required>
                        <option value="0">Chỉ cộng điểm sử dụng, không xét hạng</option>
                        <option value="1">Cộng điểm và tính vào hạng thành viên</option>
                    </select>
                </label>
        ...
        <article class="member-point-form is-remove">
            ...
            <form method="POST" action="{{ route('admin.thanh-vien.tru-diem', $thanhVien) }}"
                onsubmit="return confirm('Bạn có chắc muốn thu hồi điểm của thành viên này?')">
                @csrf
                <label>
                    <span>Số điểm</span>
                    <input type="number" name="so_diem" min="1" max="10000" placeholder="VD: 50" required>
                </label>

                <label>
                    <span>Lý do thu hồi</span>
                    <textarea name="noi_dung" rows="3" placeholder="VD: Thu hồi điểm cấp sai" required></textarea>
                </label>
