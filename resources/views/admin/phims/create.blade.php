...
<div class="movie-create-page" style="padding-top: 10px;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
        <div>
            ...
            <p style="color: #9ca3af; font-size: 14px;">Tạo phim mới, tải poster và điền đầy đủ các thông tin chi tiết.</p>
        </div>
            ...
            <div>
                ...
                <div class="movie-panel">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 16px;">
                        <div>
                            <label style="display: block; color: #d1d5db; font-size: 13px; font-weight: 600; margin-bottom: 6px;">Tên phim <span style="color: #ef4444;">*</span></label>
                            <input type="text" name="ten_phim" value="{{ old('ten_phim') }}" required placeholder="Ví dụ: Ánh Sáng Thành Phố" class="form-input-cine">
                        </div>

                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                            <div>
                                <label style="display: block; color: #d1d5db; font-size: 13px; font-weight: 600; margin-bottom: 6px;">Đạo diễn</label>
                                <input type="text" name="dao_dien" value="{{ old('dao_dien') }}" placeholder="Tên đạo diễn" class="form-input-cine">
                            </div>
                            <div>
                                <label style="display: block; color: #d1d5db; font-size: 13px; font-weight: 600; margin-bottom: 6px;">Diễn viên</label>
                                <input type="text" name="dien_vien" value="{{ old('dien_vien') }}" placeholder="Các diễn viên chính..." class="form-input-cine">
                            </div>
                        </div>

                        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 16px;">
                            <div>
                                <label style="display: block; color: #d1d5db; font-size: 13px; font-weight: 600; margin-bottom: 6px;">Thời lượng (phút) <span style="color: #ef4444;">*</span></label>
                                <input type="number" name="thoi_luong" value="{{ old('thoi_luong', 120) }}" required placeholder="120" class="form-input-cine">
                            </div>

                            <div>
                                <label style="display: block; color: #d1d5db; font-size: 13px; font-weight: 600; margin-bottom: 6px;">Giới hạn tuổi</label>
                                <div class="cine-select-wrapper">
                                    <input type="hidden" name="gioi_han_tuoi" value="{{ old('gioi_han_tuoi', 'P') }}">
                                    <div class="cine-select-trigger" tabindex="0">
                                        <span class="cine-select-value">P - Mọi lứa tuổi</span>
                                        <i class="fa-solid fa-chevron-down"></i>
                                    </div>
                                    <div class="cine-select-menu">
                                        <div class="cine-select-option {{ old('gioi_han_tuoi', 'P') == 'P' ? 'selected' : '' }}" data-value="P">P - Mọi lứa tuổi</div>
                                        <div class="cine-select-option {{ old('gioi_han_tuoi') == 'K' ? 'selected' : '' }}" data-value="K">K - Dưới 13 tuổi có phụ huynh</div>
                                        <div class="cine-select-option {{ old('gioi_han_tuoi') == 'T13' ? 'selected' : '' }}" data-value="T13">T13 - Từ 13 tuổi trở lên</div>
                                        <div class="cine-select-option {{ old('gioi_han_tuoi') == 'T16' ? 'selected' : '' }}" data-value="T16">T16 - Từ 16 tuổi trở lên</div>
                                        <div class="cine-select-option {{ old('gioi_han_tuoi') == 'T18' ? 'selected' : '' }}" data-value="T18">T18 - Từ 18 tuổi trở lên</div>
                                    </div>
                                </div>
                            </div>

                            <div>
                                <label style="display: block; color: #d1d5db; font-size: 13px; font-weight: 600; margin-bottom: 6px;">Ngôn ngữ</label>
                                <input type="text" name="ngon_ngu" value="{{ old('ngon_ngu', 'Tiếng Việt / Phụ đề tiếng Anh') }}" placeholder="Tiếng Việt / Phụ đề..." class="form-input-cine">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="movie-panel">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 16px;">
                        <div>
                            <label style="display: block; color: #d1d5db; font-size: 13px; font-weight: 600; margin-bottom: 6px;">Quốc gia <span style="color: #ef4444;">*</span></label>
                            <div class="cine-select-wrapper">
                                <input type="hidden" name="quoc_gia_id" value="{{ old('quoc_gia_id') }}" required>
                                <div class="cine-select-trigger" tabindex="0">
                                    <span class="cine-select-value">Chọn quốc gia</span>
                                    <i class="fa-solid fa-chevron-down"></i>
                                </div>
                                <div class="cine-select-menu">
                                    <div class="cine-select-option {{ empty(old('quoc_gia_id')) ? 'selected' : '' }}" data-value="">Chọn quốc gia</div>
                                    @foreach ($countryList as $qg)
                                        @php
                                            $qgId = is_object($qg) ? $qg->id : $qg;
                                            $qgName = is_object($qg) ? $qg->ten_quoc_gia : $qg;
                                        @endphp
                                        <div class="cine-select-option {{ old('quoc_gia_id') == $qgId ? 'selected' : '' }}" data-value="{{ $qgId }}">
                                            {{ $qgName }}
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        <div>
                            <label style="display: block; color: #d1d5db; font-size: 13px; font-weight: 600; margin-bottom: 8px;">Thể loại phim (Chọn nhiều)</label>
                            <div style="display: flex; flex-wrap: wrap; gap: 10px; background: #18181c; padding: 14px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.15);">
                                @foreach ($genreList as $tl)
                                    @php
                                        $tlId = is_object($tl) ? $tl->id : $tl;
                                        $tlName = is_object($tl) ? $tl->ten_the_loai : $tl;
                                    @endphp
                                    <label style="display: inline-flex; align-items: center; gap: 8px; background: rgba(255,255,255,0.06); padding: 6px 12px; border-radius: 8px; color: #fff; font-size: 13px; cursor: pointer; border: 1px solid rgba(255,255,255,0.1);">
                                        <input type="checkbox" name="the_loai_id[]" value="{{ $tlId }}" @checked(in_array($tlId, $selectedGenres)) style="accent-color: #facc15;">
                                        <span>{{ $tlName }}</span>
                                    </label>
                                @endforeach
                            </div>
                ...
                <div class="movie-panel">
                    ...
                    <textarea name="mo_ta" rows="4" placeholder="Nhập tóm tắt nội dung phim..." class="form-input-cine" style="resize: vertical;">{{ old('mo_ta') }}</textarea>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="movie-panel">
                    ...
                    <div style="text-align: center; border: 2px dashed rgba(255,255,255,0.2); padding: 20px; border-radius: 16px; background: #18181c;">
                        <img id="posterPreview" src="{{ asset('assets/images/LOGO copy.png') }}" style="max-height: 220px; border-radius: 12px; margin-bottom: 12px; object-fit: cover;">
