...
@section('page-subtitle', 'Tạo phim mới, cập nhật poster, banner, trailer và thông tin phân loại')
...
<div class="movie-create-page" style="padding-top: 10px;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
        <div>
            ...
            <p style="color: #9ca3af; font-size: 14px;">Tạo phim mới, tải poster, banner trang chủ và điền đầy đủ các thông tin chi tiết.</p>
        </div>
    ...
    <form id="createMovieForm" action="{{ route('admin.phims.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 24px;">
                ...
                <div class="movie-panel">
                    ...
                    <div style="display:flex;flex-direction:column;gap:16px;">
                        ...
                        <div>
                            ...
                            <input type="text" name="ten_phim" value="{{ old('ten_phim') }}" placeholder="Ví dụ: Ánh Sáng Thành Phố" class="form-input-cine @error('ten_phim') is-invalid @enderror">
                            @error('ten_phim')
                        ...
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                            ...
                            <div>
                                ...
                                <input type="text" name="dao_dien" value="{{ old('dao_dien') }}" placeholder="Tên đạo diễn" class="form-input-cine @error('dao_dien') is-invalid @enderror">
                                @error('dao_dien')
                                <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="dien_vien" value="{{ old('dien_vien') }}" placeholder="Các diễn viên chính..." class="form-input-cine @error('dien_vien') is-invalid @enderror">
                                @error('dien_vien')
                                <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                        ...
                        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;">
                            ...
                            <div>
                                ...
                                <input type="number" name="thoi_luong" value="{{ old('thoi_luong') }}" placeholder="120" class="form-input-cine @error('thoi_luong') is-invalid @enderror">
                                @error('thoi_luong')
                                <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <div class="cine-select-wrapper @error('gioi_han_tuoi') is-invalid @enderror">
                                    ...
                                    <div class="cine-select-trigger" tabindex="0">
                                        <span class="cine-select-value">{{ old('gioi_han_tuoi','P') }}</span>
                            ...
                            <div>
                                ...
                                <input type="text" name="ngon_ngu" value="{{ old('ngon_ngu') }}" placeholder="Tiếng Việt..." class="form-input-cine @error('ngon_ngu') is-invalid @enderror">
                                @error('ngon_ngu')
                                <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                    ...
                    <div style="display:flex;flex-direction:column;gap:16px;">
                        ...
                        <div>
                            ...
                            <div class="cine-select-wrapper">
                                <input type="hidden" name="quoc_gia_id" value="{{ old('quoc_gia_id') }}">
                                <div class="cine-select-trigger @error('quoc_gia_id') is-invalid @enderror" tabindex="0">
                                    <span class="cine-select-value">
                                        ...
                                        @endphp
                                        {{ old('quoc_gia_id') ? (is_object($selectedCountry) ? $selectedCountry->ten_quoc_gia : old('quoc_gia_id')) : 'Chọn quốc gia' }}
                                    </span>
                                    <i class="fa-solid fa-chevron-down"></i>
                                </div>
                                <div class="cine-select-menu">
                                    ...
                                    @endphp
                                    <div class="cine-select-option {{ old('quoc_gia_id') == $id ? 'selected' : '' }}" data-value="{{ $id }}">
                                        {{ $name }}
                                    </div>
                                    @endforeach
                                </div>
                        ...
                        <div>
                            ...
                            <div class="@error('the_loai_id') is-invalid @enderror" style="display:flex;flex-wrap:wrap;gap:10px;background:#18181c;padding:14px;border-radius:12px;border:1px solid rgba(255,255,255,.15);">
                                ...
                                <label style="display:inline-flex;align-items:center;gap:8px;background:rgba(255,255,255,.06);padding:6px 12px;border-radius:8px;color:#fff;font-size:13px;cursor:pointer;">
                                    <input type="checkbox" name="the_loai_id[]" value="{{ $id }}" {{ in_array($id, old('the_loai_id', [])) ? 'checked' : '' }} style="accent-color:#facc15;">
                                    <span>{{ $name }}</span>
                                </label>
                                @endforeach
                            </div>
            ...
            <!-- CỘT BÊN PHẢI: POSTER, BANNER & TRAILER -->
            <div>
                <!-- POSTER UPLOAD -->
                <div class="movie-panel">
                    <h3 style="color: #fff; font-size: 18px; font-weight: 800; margin-bottom: 16px; display: flex; align-items: center; gap: 10px;">
                    ...
                    <div style="text-align: center; border: 2px dashed rgba(255,255,255,0.2); padding: 20px; border-radius: 16px; background: #18181c;">
                        ...
                        <button type="button" onclick="document.getElementById('posterInput').click()" class="movie-action-btn is-ghost" style="width: 100%; justify-content: center;">
                            <i class="fa-solid fa-upload"></i> Chọn file poster
                        </button>
                        @error('poster')
                        <small class="text-danger d-block mt-1">{{ $message }}</small>
                        @enderror
                ...
                <div class="movie-panel">
                    ...
                    <div style="text-align: center; border: 2px dashed rgba(255,255,255,0.2); padding: 20px; border-radius: 16px; background: #18181c;">
                        <img id="bannerPreview" src="{{ asset('assets/images/LOGO copy.png') }}" style="width: 100%; max-height: 140px; border-radius: 12px; margin-bottom: 12px; object-fit: cover;">
                        ...
                        <button type="button" onclick="document.getElementById('bannerInput').click()" class="movie-action-btn is-ghost" style="width: 100%; justify-content: center;">
                            <i class="fa-solid fa-upload"></i> Chọn file banner ngang
                        </button>
                ...
                <div class="movie-panel">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 12px;">
                        <div>
                            ...
                            <input type="url" name="trailer" id="trailerInput" value="{{ old('trailer', old('trailer_url')) }}" placeholder="https://www.youtube.com/watch?v=..." class="form-input-cine" autocomplete="off">
                            @error('trailer')
                            <small class="text-danger d-block mt-1">{{ $message }}</small>
                            @enderror
