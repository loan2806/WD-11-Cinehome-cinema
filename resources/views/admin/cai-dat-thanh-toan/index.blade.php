...
@section('page-subtitle', 'Bật tắt phương thức thanh toán dùng trong luồng đặt vé')
...
<form method="POST" action="{{ route('admin.cai-dat-thanh-toan.update') }}" class="space-y-6">
    @csrf
    @method('PATCH')

    <section class="admin-panel">
        <div class="panel-header">
            <h5>Phương thức khả dụng</h5>
        </div>
                    ...
                    <label class="flex items-center justify-between gap-4 rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-4">
                        <span>
                            ...
                            <span class="text-xs text-gray-500">
                                {{ (string) ($settings[$key]->gia_tri ?? '0') === '1' ? 'Đang bật' : 'Đang tắt' }}
                            </span>
                        </span>
