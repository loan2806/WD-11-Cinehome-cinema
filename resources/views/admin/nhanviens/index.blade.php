    ...
    <section class="staff-list-hero">
        <div class="staff-list-hero-copy">
            <span class="staff-list-kicker">
                <i class="fa-solid fa-user-shield"></i> Quản trị nhân sự
            </span>
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('admin.nhanviens.trash') }}" class="staff-list-secondary-btn">
    ...
    <section class="staff-list-panel">
        ...
        <form method="GET" action="{{ route('admin.nhanviens.index') }}" class="staff-list-filter">
            <label class="staff-list-search">
                ...
                <input type="text" name="keyword" value="{{ request('keyword') }}" placeholder="Tìm theo tên hoặc email...">
            </label>

            <div class="staff-status-dropdown" data-dropdown>
                <input type="hidden" name="status" value="{{ request('status') }}" data-dropdown-value>

                <button type="button" class="staff-dropdown-trigger" data-dropdown-trigger>
                ...
                <div class="staff-dropdown-menu" data-dropdown-menu>
                    <button type="button" class="staff-dropdown-option {{ !request('status') ? 'is-selected' : '' }}" data-value="">
                        <i class="fa-solid fa-filter status-all-icon"></i>
                        <span>Tất cả trạng thái</span>
                    </button>
                    <button type="button" class="staff-dropdown-option {{ request('status') === 'active' ? 'is-selected' : '' }}" data-value="active">
                        <i class="fa-solid fa-circle-check status-active-icon"></i>
                        <span>Đang hoạt động</span>
                    </button>
                    <button type="button" class="staff-dropdown-option {{ request('status') === 'locked' ? 'is-selected' : '' }}" data-value="locked">
                        <i class="fa-solid fa-lock status-locked-icon"></i>
                        <span>Đã khóa</span>
                    </button>
                </div>
            ...
            <table class="staff-list-table">
                ...
                <tbody>
                    @forelse($nhanViens as $nhanVien)
                        ...
                        @endphp
                        <tr>
                            <td><span class="staff-id-badge">#{{ $nhanVien->id }}</span></td>
                            <td>
                                <div class="staff-profile-cell">
                                    ...
                                    <div>
                                        ...
                                        <small>
                                            <i class="fa-solid fa-briefcase"></i> Nhân viên hệ thống
                                            @if ($isSelf)
                            ...
                            <td><span class="staff-email">{{ $nhanVien->email }}</span></td>
                            <td>
                                ...
                                <div class="staff-actions">
                                    ...
                                    @if($nhanVien->trashed())
                                        <form method="POST" action="{{ route('admin.nhanviens.restore', $nhanVien->id) }}">
                                            @csrf
                                            <button class="staff-action-btn is-unlock" type="submit" title="Khôi phục nhân viên">
                                                <i class="fa-solid fa-rotate-left"></i>
                                            </button>
                                        </form>
                                    @else
                                        <form method="POST" action="{{ route('admin.nhanviens.toggle-status', $nhanVien) }}">
                                            @csrf
                                            @method('PATCH')
                                            <button class="staff-action-btn {{ $isActive ? 'is-lock' : 'is-unlock' }}"
                                                    type="submit"
        ...
        <div class="staff-list-pagination">
            {{ $nhanViens->withQueryString()->links() }}
        </div>
    </section>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('[data-dropdown]').forEach(function (dropdown) {
        ...
        function openDropdown() {
            ...
            requestAnimationFrame(calculateDirection);
        }
        ...
        menu.querySelectorAll('[data-value]').forEach(function (option) {
            option.addEventListener('click', function (e) {
                ...
                dropdown.querySelector('.staff-dropdown-current').innerHTML = icon + '<span>' + text + '</span>';

                menu.querySelectorAll('.staff-dropdown-option').forEach(item => item.classList.remove('is-selected'));
        ...
        window.addEventListener('resize', () => dropdown.classList.contains('is-open') && calculateDirection());
        window.addEventListener('scroll', () => dropdown.classList.contains('is-open') && calculateDirection(), true);
    });
});
</script>
@endsection