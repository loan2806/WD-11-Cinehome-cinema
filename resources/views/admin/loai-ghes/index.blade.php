...
@section('title', 'Quản lý Loại Ghế - CineHome')
            ...
            <div class="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-[#1e1e1e] to-[#121212] p-5 shadow-xl transition-all duration-300 hover:scale-[1.02] hover:border-red-500/40 group">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-4xl font-black text-white mt-2">{{ $totalSeatTypes }}</h3>
                    </div>
            ...
            <div class="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-[#1e1e1e] to-[#121212] p-5 shadow-xl transition-all duration-300 hover:scale-[1.02] hover:border-red-500/40 group">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-4xl font-black text-white mt-2">{{ number_format($totalSeats) }}</h3>
                    </div>
                    <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-red-500/10 text-red-500 border border-red-500/20">
            ...
            <div class="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-[#1e1e1e] to-[#121212] p-5 shadow-xl transition-all duration-300 hover:scale-[1.02] hover:border-red-500/40 group">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-4xl font-black text-red-500 mt-2">+{{ number_format($maxSurcharge) }}đ</h3>
                    </div>
                    <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-red-500/10 text-red-500 border border-red-500/20">
        ...
        <div id="seatTypeGridView" class="hidden grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse ($loaiGhes as $loai)
                <div class="group relative flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-b from-[#1c1c1c] to-[#121212] p-5 transition-all duration-300 hover:-translate-y-1 hover:border-red-500/30 hover:shadow-[0_10px_35px_rgba(229,9,20,0.15)]">
                    ...
                    {{-- Visual Ghế Mô Phỏng Phát Sáng Màu Sắc Đặc Trưng --}}
                    <div class="relative h-24 rounded-xl flex items-center justify-center overflow-hidden bg-black/30 border border-white/5 mb-5 transition-transform duration-500 group-hover:scale-[1.02]">
                        ...
                        <div class="absolute h-12 w-12 rounded-full blur-2xl opacity-35" style="background-color: {{ $loai->mau_sac ?? '#666666' }}"></div>
                        
                        <!-- Mô hình 3D vector ghế xem phim -->
                        <div class="relative flex flex-col items-center">
                            <!-- Tựa lưng -->
                            <div class="w-12 h-9 rounded-t-xl border-t border-x border-white/25 relative flex items-center justify-center transition-all duration-300 group-hover:scale-105" style="background-color: {{ $loai->mau_sac ?? '#666666' }}">
                                <span class="text-[8px] font-black tracking-widest text-white/50 select-none drop-shadow-lg">CINE</span>
                            </div>
                            <!-- Đệm ngồi -->
                            <div class="w-13 h-4 rounded-b-lg border-b border-x border-white/30 mt-0.5" style="background-color: {{ $loai->mau_sac ?? '#666666' }}"></div>
                            <!-- Tay vịn 2 bên -->
                    ...
                    <div class="flex-grow flex flex-col">
                        ...
                        <p class="text-xs text-gray-400 mt-2.5 line-clamp-2 flex-grow">
                            {{ $loai->mo_ta ?? 'Chưa có mô tả chi tiết cho loại ghế này.' }}
                        </p>
                        ...
                        <div class="mt-6 pt-4 border-t border-white/5 flex items-center justify-between gap-2">
                            ...
                            <form action="{{ route('admin.loai-ghes.destroy', $loai) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    ...
                                    title="{{ $loai->ghe_ngois_count > 0 ? 'Không thể xóa loại ghế đang có ghế hoạt động' : 'Xóa loại ghế' }}">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <span class="inline-block h-8 w-8 rounded-lg border border-white/15 shadow-md transition group-hover:scale-105"
                                              style="background-color: {{ $loai->mau_sac ?? '#666666' }}; box-shadow: 0 0 10px {{ ($loai->mau_sac ?? '#666666') . '33' }};"></span>
                                    </div>
                                </td>

                                {{-- TÊN LOẠI --}}
                                <td class="px-6 py-4">
                                    <span class="text-white font-extrabold text-base tracking-wide">{{ $loai->ten_loai }}</span>
                                </td>
