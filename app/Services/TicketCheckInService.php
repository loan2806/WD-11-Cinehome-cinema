...
{
    /**
     * Kiểm tra vé trước khi cho khách vào rạp.
     *
     * Luồng hợp lệ:
     * da_thanh_toan -> da_in -> da_su_dung
     *
     * Chỉ vé da_in mới được phép xác nhận sử dụng.
     */
    public function inspect(string $rawCode): array
    {
        ...
        if ($ticket->trang_thai === 'da_huy') {
            return $this->failed(
                'Vé này đã bị hủy, không thể sử dụng.',
                $ticket
            );
        }
        ...
        if ($ticket->trang_thai === 'da_thanh_toan') {
            return $this->failed(
                'Vé đã thanh toán nhưng chưa được in. Vui lòng in vé trước khi soát.',
                $ticket
            );
        }
        ...
        if ($ticket->trang_thai !== 'da_in') {
            return $this->failed(
                'Trạng thái vé không hợp lệ để soát.',
                $ticket
            );
        }
        ...
        return [
            'success' => true,
            'message' => 'Vé hợp lệ và đã được in. Có thể xác nhận khách vào rạp.',
            'ticket' => $ticket,
            'ma_ve' => $ticket->ma_ve,
        ];
    }
    ...
    /**
     * Đóng gói dữ liệu vé gửi về client qua AJAX.
     */
    public function ticketPayload(
        ?VeXemPhim $ticket,
        ?array $foods = null
    ): ?array {
        if (! $ticket) {
        ...
        return [
            ...
            'trang_thai_label' =>
                $this->statusLabel($ticket->trang_thai),
            'loai_ve_label' =>
                $this->typeLabel($ticket->loai_ve),

            // Chỉ vé đã in mới được xác nhận sử dụng.
            'can_check_in' =>
    ...
    private function findTicket(
        ...
        if ($maVe === '') {
            return $this->failed(
                'Mã QR không hợp lệ. Vui lòng quét lại hoặc nhập mã vé.'
            );
        }

        $ticket = VeXemPhim::where(
            'ma_ve',
            $maVe
        )->first();
