...
public function index(): View
{
    ...
    // 1. Phim mới cập nhật trong 30 ngày gần đây
    $phimMoiCapNhatCount = Phims::where(
        'created_at',
        '>=',
    ...
    $suatChieuHomNayCount = SuatChieu::whereDate(
        'thoi_gian_chieu',
        $today
    )
    ...
    // 3. Doanh thu vé hôm nay & hôm qua
    ...
    // Tỷ lệ tăng trưởng doanh thu vé so với hôm qua
    $phanTramTangTruong = 0;

    if ($doanhThuVeHomQua > 0) {
        $phanTramTangTruong = round(
            (($doanhThuVeHomNay - $doanhThuVeHomQua) / $doanhThuVeHomQua) * 100
        );
    ...
    // 6. Doanh thu đồ ăn hôm nay
    $doanhThuDoAnHomNay = DB::table('food_invoices')
        ->where('payment_status', 'paid')
        ->whereDate('created_at', $today)
        ->sum('total');

    // 7. Danh sách 5 phim mới cập nhật
