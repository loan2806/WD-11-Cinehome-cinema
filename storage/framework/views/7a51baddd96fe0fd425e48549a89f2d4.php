    ...
    <div class="container mx-auto px-4 md:px-6">
        ...
        <div class="flex flex-wrap justify-center gap-2 mb-10">
            <a href="<?php echo e(route('user.tin-tuc.index')); ?>"
                ...
                <?php echo e(!request('danh_muc') ? 'border-[#d99a32] bg-[#d99a32]/20 text-[#d99a32]' : 'border-white/10 bg-white/5 text-gray-400 hover:border-white/30 hover:bg-white/10'); ?>">
            ...
            <a href="<?php echo e(route('user.tin-tuc.index', ['danh_muc' => $danhMuc->slug])); ?>"
                class="pagination-btn px-5 py-2.5 text-sm font-semibold border transition-all
                <?php echo e(request('danh_muc') === $danhMuc->slug ? 'border-[#d99a32] bg-[#d99a32]/20 text-[#d99a32]' : 'border-white/10 bg-white/5 text-gray-400 hover:border-white/30 hover:bg-white/10'); ?>">
                <i class="<?php echo e($danhMuc->icon ?? 'fa-solid fa-tag'); ?> mr-2"></i><?php echo e($danhMuc->ten_danh_muc); ?>
        ...
        <div class="mb-12">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <?php $__currentLoopData = $tinNoiBat; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $tin): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php if($key === 0): ?>
                
                <a href="<?php echo e(route('user.tin-tuc.show', $tin->slug)); ?>"
                    class="relative rounded-2xl overflow-hidden group col-span-1 lg:row-span-1" style="min-height: 400px;">
                    
                    <div class="absolute inset-0 z-0">
                        <?php if($tin->hinh_anh && file_exists(public_path('storage/' . $tin->hinh_anh))): ?>
                            <img src="<?php echo e(asset('storage/' . $tin->hinh_anh)); ?>"
                                alt="<?php echo e($tin->tieu_de); ?>"
                    ...
                    <div class="absolute bottom-0 left-0 right-0 z-20 p-6">
                        ...
                        <span class="inline-block px-3 py-1 rounded-full bg-[#d99a32]/20 border border-[#d99a32]/50 text-[#d99a32] text-xs font-bold">
                            <i class="<?php echo e($tin->danhMucTin->icon ?? 'fa-solid fa-tag'); ?> mr-1"></i>
                            <?php echo e($tin->danhMucTin->ten_danh_muc); ?>

                        </span>
                        <?php endif; ?>
                        <h3 class="mt-3 text-xl lg:text-2xl font-bold text-white group-hover:text-[#d99a32] transition-colors line-clamp-2">
                            <?php echo e($tin->tieu_de); ?>

                        </h3>
                        <p class="mt-2 text-sm text-gray-300 line-clamp-2">
                            <?php echo e($tin->mo_ta_ngan); ?>

                        </p>
                        <div class="mt-4 flex items-center gap-4 text-xs text-gray-400">
                            <span><i class="fa-solid fa-eye mr-1"></i><?php echo e(number_format($tin->luot_xem)); ?></span>
                            <span><i class="fa-solid fa-calendar mr-1"></i><?php echo e($tin->ngay_dang ? $tin->ngay_dang->format('d/m/Y') : now()->format('d/m/Y')); ?></span>
                        </div>
                    </div>
                </a>
                <?php else: ?>
                ...
                <a href="<?php echo e(route('user.tin-tuc.show', $tin->slug)); ?>"
                    ...
                    <div class="flex flex-col justify-center flex-1 min-w-0">
                        ...
                        <p class="mt-1 text-xs text-gray-500">
                            <?php echo e($tin->ngay_dang ? $tin->ngay_dang->format('d/m/Y') : now()->format('d/m/Y')); ?>
                ...
                <div class="voucher-card p-5">
                    <div class="flex items-start justify-between mb-3">
                        <span class="px-3 py-1 rounded-lg bg-purple-500/20 border border-purple-500/30 text-purple-400 text-xs font-bold">
                            <i class="fa-solid fa-ticket mr-1"></i><?php echo e($voucher->ma_voucher); ?>

                        </span>
                        <?php if($voucher->ngay_het_han): ?>
                            <?php $daysLeft = now()->diffInDays($voucher->ngay_het_han, false); ?>
                            <?php if($daysLeft <= 7 && $daysLeft > 0): ?>
                                <span class="px-2 py-1 rounded-lg bg-red-500/20 border border-red-500/30 text-red-400 text-xs font-bold">
                                    <i class="fa-solid fa-clock mr-1"></i><?php echo e(floor($daysLeft)); ?> ngày
                                </span>
                            <?php endif; ?>
                        <?php endif; ?>
                    </div>

                    <h3 class="text-base font-bold text-white line-clamp-2"><?php echo e($voucher->ten_voucher); ?></h3>

                    <div class="mt-3 p-3 rounded-xl bg-black/30 border border-white/10 text-center">
                        <p class="text-xs text-gray-500">Giảm</p>
                        <p class="text-2xl font-black text-purple-400"><?php echo e(number_format($voucher->gia_tri_giam, 0, ',', '.')); ?>đ</p>
                    </div>

                    <?php if($voucher->ngay_het_han): ?>
                    <p class="mt-3 text-xs text-gray-500">
                        <i class="fa-solid fa-calendar-xmark mr-1"></i>
                        Hạn: <?php echo e(\Carbon\Carbon::parse($voucher->ngay_het_han)->format('d/m/Y')); ?>

                    </p>
                    <?php endif; ?>

                    <button onclick="suDungVoucher(<?php echo e($voucher->id); ?>)"
        ...
        <div class="mb-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                ...
                <form method="GET" action="<?php echo e(route('user.tin-tuc.index')); ?>" class="flex items-center gap-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <a href="<?php echo e(route('user.tin-tuc.show', $tin->slug)); ?>" class="news-card group">
            ...
            <div class="mt-10 flex justify-center">
                <div class="flex items-center gap-2">
                    ...
                    <?php else: ?>
                        <a href="<?php echo e($tinTucs->previousPageUrl()); ?>" class="pagination-btn border border-white/10 bg-white/5 text-gray-400 hover:bg-white/10">
                            <i class="fa-solid fa-chevron-left"></i>
                        </a>
                    <?php endif; ?>
                    ...
                    <?php $__currentLoopData = $tinTucs->getUrlRange(max(1, $tinTucs->currentPage() - 1), min($tinTucs->lastPage(), $tinTucs->currentPage() + 1)); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $page => $url): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <?php else: ?>
                            <a href="<?php echo e($url); ?>" class="pagination-btn border border-white/10 bg-white/5 text-gray-400 hover:bg-white/10">
                                <?php echo e($page); ?>
                    ...
                    <?php if($tinTucs->hasMorePages()): ?>
                        <a href="<?php echo e($tinTucs->nextPageUrl()); ?>" class="pagination-btn border border-white/10 bg-white/5 text-gray-400 hover:bg-white/10">
...
<?php echo $__env->make('layouts.user', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\WD-11-Cinehome-cinema\resources\views/user/tin-tuc/index.blade.php ENDPATH**/ ?>